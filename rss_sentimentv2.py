...
if feed.entries:
    ...
    # Sort news items by sentiment score (highest to lowest)
    news_items.sort(key=lambda x: x['compound_score'], reverse=True)

    # Calculate total sentiment score for the top 5 news items
    top_5_news = news_items[:5]
    total_sentiment_score = sum(item['compound_score'] for item in top_5_news)
    ...
    for item in top_5_news:  # Display the top 5 news items by sentiment
